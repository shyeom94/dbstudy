...
SELECT CONCAT(CONCAT(SUBSTR(NAME, 1, 1), '*'), SUBSTR(NAME, 3, 1)) AS 이름
  FROM EMPLOYEE_T;
...
-- 7. 채우기
-- 1) LPAD(EXPR1, N, [EXPR2]) : N자리의 EXPR1을 반환,   왼쪽에 EXPR2를 채움(생략하면 공백으로 채움)
-- 2) RPAD(EXPR1, N, [EXPR2]) : N자리의 EXPR1을 반환, 오른쪽에 EXPR2를 채움(생략하면 공백으로 채움)
...
-- 3)  TRIM(EXPR) : 왼쪽/오른쪽 공백 제거 --중간 공백은 제거하지 않음
