...
/*
    ...
    4. 칼럼 이름 바꾸기   : ALTER TABLE 테이블명 RENAME COLUMN 기존칼럼 TO 신규칼럼 
    5. TABLE 이름 바꾸기  : ALTER TABLE 테이블명 RENAME TO     신규 테이블명
*/

-- 테이블을 만드는 것   -> 칼럼을 만드는 것 
-- 테이블을 수정하는 것 -> 칼럼을 수정하는 것 

-- 1. BANK_T에 연락처(BANK_TEL) 칼럼 추가하시오. 
...
ALTER TABLE BANK_T MODIFY BANK_NAME VARCHAR2(15 BYTE) NOT NULL; --필수 제약조건 지정 => NOT NULL
...
ALTER TABLE CUSTOMER_T MODIFY CUST_NAME VARCHAR2(30 BYTE) NULL; -- 필수 제약조건 삭제 
...
/*
    ...
    2. FK
        1) 추가       : ALTER TABLE 자식테이블명 ADD     CONSTRAINT 제약조건명 FOREIGN KEY(칼럼명) REFERENCES 부모테이블명(참조할칼럼) [옵션 추가 가능]
