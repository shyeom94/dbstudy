...
SELECT EMP_NO, NAME, DEPART, POSITION, GENDER, HIRE_DATE, SALARY
  FROM (SELECT RANK() OVER(ORDER BY SALARY DESC) AS RN, EMP_NO, NAME, DEPART, POSITION, GENDER, HIRE_DATE, SALARY -- RANK() VS ROW_NUM() 동점자 중복 처리 함수 
          FROM EMPLOYEE_T)
 WHERE RN BETWEEN 2 AND 3;           

-- 2. 연봉 2 ~ 3위 사원 정보를 조회하시오. (WITH 절 이용)
WITH RANKING_T -- WITH 절
AS (SELECT RANK() OVER(ORDER BY SALARY DESC) AS RN, EMP_NO, NAME, DEPART, POSITION, GENDER, HIRE_DATE, SALARY -- RANK() VS ROW_NUM() 동점자 중복 처리 함수 
      FROM EMPLOYEE_T)
SELECT EMP_NO, NAME, DEPART, POSITION, GENDER, HIRE_DATE, SALARY
  FROM RANKING_T
 WHERE RN BETWEEN 2 AND 3; 
...
-- 4. 부서별 부서번호, 부서명, 연봉총액을 조회하시오. (WITH 절 이용)
WITH GROUP_T
