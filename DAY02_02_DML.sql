...
/*
    ...
    3. 작업의 저장을 의미하는 COMMIT과 작업의 취소를 의미하는 ROLLBACK이 필요하다. 반드시 COMMIT 해야 저장된다. -> TCL (COMMIT, ROLLBACK)
    4. 예시 : 은행 이체 
        1) 내 통장에서 돈 빼기 
        2) 너 통장으로 돈 넣기 
*/
...
--DROP TABLE EMPLOTEE_T;
--DROP TABLE DEPARTMENT_T;
--DROP TABLE EMPLOYEE_T;

-- 시퀀스 삭제 
DROP SEQUENCE DEPT_SEQ;
DROP SEQUENCE EMPLOTEE_SEQ;
...
--                      (                            ) => 생략 가능할 때 : 모든 칼럼을 작성하는 경우 

-- 사원 테이블에 행 삽입하기
INSERT INTO EMPLOYEE_T VALUES(EMPLOYEE_SEQ.NEXTVAL, '구창민', 1, '과장', 'M', '95/05/01', 5000000); -- 날짜는 하이픈(-) 또는 슬래시(/)로 표기
...
COMMIT; --> 반드시 꼭 커밋까지 해야 적용되는 것이다.

-- 2. EMPLOYEE_T에서 부서번호(DEPART)가 2인 부서의 사원들의 기본급(SALARY)를 10% 인상하시요.
UPDATE EMPLOYEE_T
...
/*
    ...
    DELETE FROM 테이블명 WHERE 조선식
*/

-- 1. DEPARTMENT_T에서 부서번호(DEPT_NO)가 3인 부서를 삭제하시오. (부서번호가 3인 사원은 없다. 참조 무결성에 영항을 미치지 않는다.)
DELETE 
  FROM DEPARTMENT_T 
 WHERE DEPT_NO = 3;
...
-- 3. DEPARTMENT_T에서 부서번호(DEPT_NO)RK 2인 부서를 삭제하시오. (부서번호가 2인 사원이 있다. 부서가 없어지면 사원 정보가 참조 무결성에 위배된다. 하지만 )
DELETE 
  FROM DEPARTMENT_T
 WHERE DEPT_NO = 2;
