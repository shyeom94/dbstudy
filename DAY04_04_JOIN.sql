...
SELECT E.EMP_NO
  ...
  FROM DEPARTMENT_T D INNER JOIN EMPLOYEE_T E 
    ON D.DEPT_NO = E.DEPART; -- JOIN 할 때 별명은 꼭 붙이자!
...
SELECT D.DEPT_NAME
     , AVG(E.SALARY) 
  FROM DEPARTMENT_T D INNER JOIN EMPLOYEE_T E
    ON DEPT_NO = E.DEPART -- 좌측 : 검색 칼럼 DRIVE TABLE , 우측 : 값 DRIVEN TABLE
 GROUP BY D.DEPT_NAME;

/*
    외부 조인(OUTER JOIN)
    1. 어느 한 테이블에만 존재하는 데이터도 조회된다.
    2. 해당 테이블이 왼쪽에 있으면 왼쪽 외부 조인, 오른쪽에 있으면 오른쪽 외부 조인이다.
    3. ANSI 문법
        1) 왼쪽 외부 조인
        SELECT 조회할 칼럼, ...
          FROM 테이블 1 LEFT [OUTER] JOIN 테이블2 
            ON 조인조건
    
        2) 오른쪽 외부 조인
        SELECT 조회할 칼럼, ...
          FROM 테이블 1 RIGHT [OUTER] JOIN 테이블2 
            ON 조인조건
*/
...
SELECT E.EMP_NO
     ...
     , D.DEPT_NAME
  FROM DEPARTMENT_T D INNER JOIN EMPLOYEE_T E
    ON D.DEPT_NO = E.DEPART; -- 홍길동 X

SELECT E.EMP_NO
     , E.NAME
     , D.DEPT_NAME
  FROM DEPARTMENT_T D RIGHT JOIN EMPLOYEE_T E
    ON D.DEPT_NO = E.DEPART; -- 홍길동 O 
...
SELECT D.DEPT_NAME
  ...
  FROM DEPARTMENT_T D LEFT JOIN EMPLOYEE_T E 
    ON D.DEPT_NO = E.DEPART; -- 그룹핑 전 단순 조회
