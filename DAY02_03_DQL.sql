/*
    ...
    5. 형식
        SELECT 조회할 칼럼, 조회할 칼럼, ... 
          FROM 테이블명 ----------------------------여기까지 필수 입력
         WHERE 조건식
      GROUP BY 그룹화할 칼럼, 그룹화할 칼럼, ...
        HAVING 그룹조건식 
      ORDER BY 정렬할 칼럼 ASC|DESC, 정렬할 칼럼 ASC|DESC, ...   
*/

-- 1. 부서 칼럼의 모든 칼럼을 조회하시오
SELECT DEPT_NO, DEPT_NAME, LOCATION
  FROM DEPARTMENT_T;
  
SELECT * -- 모든 칼럼을 의미하는 *, 실제 개발할때는 사용 금지 -> 성능이 떨어짐 
  FROM DEPARTMENT_T;
...
-- 부서 테이블의 모든 칼럼을 조회하시오. 조회할 칼럼명을 '부서번호', '부서명', '위치'로 조회하시오. 
...
SELECT * 
  FROM DEPARTMENT_T
 WHERE DEPT_NO = 1; -- => PK 이므로 UNIQUE 함. 조회 결과는 무조건 1개 이하
...
/*
    ...
    2. 종류 
        ...
        2) _ : 1글자를 의미하는 만능문자
    3. 예시) 
        1) 첫번째 글자가 A인 모든 데이터 : A%
        2) 두번째 글자가 A인 모든 데이터 : _A%
...
SELECT * 
  FROM EMPLOYEE_T
 WHERE HIRE_DATE LIKE '%/09/%'; -- '___09___';
